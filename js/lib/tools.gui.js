...
/***/ function(module, exports, __webpack_require__) {
	...
	var _initDatGui = function _initDatGui(gui) {
	  ...
	  var funcs = {
	    Agregar: function addBody() {
	      _addBody(gui);
	    },
	    limpiar: function clear() {
	      _clear(gui);
	    },
	    save: function save() {
	      Serializer.saveState(gui.serializer, engine, prompt('Ingresa el nombre del mundo a guardar'));Events.trigger(gui, 'save');
	    },
	    load: function load() {
	      Serializer.loadState(gui.serializer, engine, prompt('Ingresa el nombre del mundo a cargar'));Events.trigger(gui, 'load');
	    }
	  };
	  ...
	  if (engine) {
	    var controls = datGui.addFolder('Agregar Cuerpo');
	    controls.add(gui, 'cantidad', 1, 5).step(1);
	    controls.add(gui, 'tamaño', 5, 150).step(1);
	    controls.add(gui, 'vértices', 1, 8).step(1);
	    controls.add(gui, 'densidad', 0.0001, 0.01).step(0.001);
	    controls.add(gui, 'fricción', 0, 1).step(0.05);
	    controls.add(gui, 'fricciónEstática', 0, 10).step(0.1);
	    controls.add(gui, 'friccionAire', 0, gui.friccionAire * 10).step(gui.friccionAire / 10);
	    controls.add(gui, 'restitución', 0, 1).step(0.1);
	    controls.add(gui, 'redondeado', 0, 30).step(2);
	    controls.add(funcs, 'Agregar');
	    controls.open();

	    var worldGui = datGui.addFolder('Escenario');
	    ...
	    worldGui.add(funcs, 'limpiar');
	    worldGui.open();

	    var gravity = worldGui.addFolder('Gravedad');
	    var ola = gravity.add(gui, 'escala', 0, 0.001).step(0.0001);
	    ola.onChange(function() {
	    	engine.world.gravity.scale = this.getValue()
	    });
	    ...
	    physics.add(engine.timing, 'timeScale', 0, 1.5).step(0.05).listen();
	    ...
	    if (runner) {
	      var activateEngine = physics.add(gui, 'activado');
	      activateEngine.onChange(function() {
	      	runner.enabled = this.getValue();
	      })
	    }
	  ...
	  if (gui.render) {
	    ...
	    var wireframes = render.add(gui, 'mostrarPlanos')
	    console.log(wireframes)
	    wireframes.onFinishChange(function (value) {
	    	if(!value) {
	    		angleIndicatorWidget.setValue(false)
	    	}
	    	gui.render.options.wireframes = value;
	    })

	    var velocity = render.add(gui, 'mostrarVelocidad');
	    velocity.onFinishChange(function (value) {
	    	gui.render.options.showVelocity = value;
	    })
	    var angleIndicatorWidget = render.add(gui, 'mostrarAngulo');
	    angleIndicatorWidget.onFinishChange(function (value) {
	    	gui.render.options.showAngleIndicator = value;
	    })
	    render.open();
	  }
	...
	var _addBody = function _addBody(gui) {
	  ...
	  var options = {
	    density: gui.densidad,
	    friction: gui.fricción,
	    frictionStatic: gui.fricciónEstática,
	    frictionAir: gui.friccionAire,
	    restitution: gui.restitución
	  };

	  if (gui.redondeado && gui.vértices > 2) {
	    options.chamfer = {
	      radius: gui.redondeado
	    };
	  }

	  for (var i = 0; i < gui.cantidad; i++) {
	    World.add(engine.world, Bodies.polygon(gui.offset.x + 120 + i * gui.tamaño + i * 50, gui.offset.y + 200, gui.vértices, gui.tamaño, options));
	  }
	};
